...
#[derive(Debug, Error)]
pub enum Error {
    #[error("provider conformance failure: {0}")]
    Conformance(String),
    #[error("core error: {0}")]
...
pub async fn check_provider(
    ...
    if descriptor.name.trim().is_empty() {
        return Err(Error::Conformance("descriptor name is empty".into()));
    }
    if descriptor.schemes.is_empty() {
        return Err(Error::Conformance("provider advertises no schemes".into()));
    }
    if !descriptor.capabilities.resolve {
        return Err(Error::Conformance(
            "provider must advertise resolve capability".into(),
        ));
    }
    let resolution = provider.resolve(requirement, context).await?;
    if resolution.provider != descriptor.name {
        return Err(Error::Conformance(format!(
            "resolution provider `{}` != descriptor `{}`",
            resolution.provider, descriptor.name
        )));
    }
    if resolution.canonical_ref.trim().is_empty() {
        return Err(Error::Conformance("canonical_ref is empty".into()));
    }
    if options.require_nonempty && resolution.files.is_empty() {
        return Err(Error::Conformance(
            "resolution unexpectedly contains no files".into(),
        ));
    }
    let mut paths = BTreeSet::new();
    for file in &resolution.files {
        if !paths.insert(file.path.to_string()) {
            return Err(Error::Conformance(format!(
                "duplicate resolved path `{}`",
                file.path
            )));
        }
        if !requirement.selection.matches(file.path.as_str())? {
            return Err(Error::Conformance(format!(
                "resolved path `{}` violates requirement selection",
                file.path
            )));
        }
        if descriptor.capabilities.acquire {
            let _ = provider
                .prepare_acquisition(
                    file,
                    &artifactum_resolver::AcquireContext {
    ...
    if options.stable_resolution {
        ...
        if canonical_json(&resolution)? != canonical_json(&again)? {
            return Err(Error::Conformance(
                "immediate repeated resolution changed".into(),
            ));
        }
    }
    let listed_files = if descriptor.capabilities.list {
        ...
        if listed != paths {
            return Err(Error::Conformance(
                "list_files does not agree with resolve".into(),
            ));
        }
        Some(page.items.len())
    } else {
    ...
    if descriptor.capabilities.inspect {
        let inspected = provider.inspect(&requirement.reference, context).await?;
        if inspected.reference.scheme().is_empty() {
            return Err(Error::Conformance(
                "inspect returned invalid reference".into(),
            ));
        }
    }
    Ok(ConformanceReport {
        ...
        listed_files,
        stable_resolution_checked: options.stable_resolution,
    })
}
...
pub async fn acquire_case(
    ...
    let descriptor = provider.descriptor();
    let resolver = artifactum_resolver::ArtifactResolver::builder()
        .store(store.clone())
        .metadata(metadata.clone())
        .provider_dyn(provider)?
        .build()
        .await?;
    ...
    if observations == 0 {
        return Err(Error::Conformance(
            "acquired artifact has no source observation".into(),
        ));
    }
...
pub fn dynamic<P: ArtifactProvider>(provider: P) -> DynProvider {
    Arc::new(provider)
}
