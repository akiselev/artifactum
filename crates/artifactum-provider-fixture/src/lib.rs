...
use artifactum_resolver::{
    ...
    ProviderCapabilities, ProviderDescriptor, Resolution, ResolveContext, ResolvedFile, Result,
};
...
impl ArtifactProvider for Fixture {
    ...
    async fn resolve(&self, r: &ArtifactRequirement, _: &ResolveContext) -> Result<Resolution> {
        let path = PathBuf::from(r.reference.locator());
        ...
        Ok(Resolution {
            ...
            canonical_ref: format!("fixture:{}", path.display()),
            revision: None,
            ...
            provider_state: serde_json::json!({"fixture":true}),
...
mod tests {
    ...
    async fn passes_provider_conformance_and_real_acquisition() {
        ...
        tokio::fs::write(&source, b"provider conformance\n")
            .await
            .unwrap();
        let requirement =
            ArtifactRequirement::new(format!("fixture:{}", source.display()).parse().unwrap());
        ...
        let acquired = acquire_case(dynamic(Fixture), requirement).await.unwrap();
        assert_eq!(acquired.source_observations, 1);
    }
}
